...
public class OlapMB extends BaseMB {
   ...
   private void createBarModel() {
      ...
      yAxis.setMax((Long)Collections.max(times) + 20L);
      this.initBarModel();
   }
   ...
   private Long executeNoSql(HashMap<String, HashMap<String, String>> tables2Columnvalue, List<String> columnsSelected2) {
      HashMap map;
      if (tables2Columnvalue.keySet().contains(Tables.TAGS.getName()) || tables2Columnvalue.keySet().contains(Tables.CATEGORY.getName())) {
         map = new HashMap();
         if (!tables2Columnvalue.keySet().contains(Tables.USER.getName())) {
            String[] var7;
            int var6 = (var7 = Tables.valueOf(Tables.USER.getName()).getCloumns()).length;

            for(int var5 = 0; var5 < var6; ++var5) {
               String col = var7[var5];
               if (!col.equalsIgnoreCase("id")) {
                  map.put(col, (Object)null);
               }

               if (col.equalsIgnoreCase(Tables.CATEGORY.getName())) {
                  map.put(col, (String)((HashMap)tables2Columnvalue.get(Tables.CATEGORY.getName())).get(col));
               }

               if (col.equalsIgnoreCase(Tables.TAGS.getName())) {
                  map.put(col, (String)((HashMap)tables2Columnvalue.get(Tables.TAGS.getName())).get(col));
               }
            }
         } else if (tables2Columnvalue.keySet().contains(Tables.USER.getName())) {
            if (tables2Columnvalue.containsKey(Tables.CATEGORY.getName())) {
               ((HashMap)tables2Columnvalue.get(Tables.USER.getName())).put(Tables.CATEGORY.getName(), (String)((HashMap)tables2Columnvalue.get(Tables.CATEGORY.getName())).get(Tables.CATEGORY.getCloumns()[1]));
            }

            if (tables2Columnvalue.containsKey(Tables.TAGS.getName())) {
               ((HashMap)tables2Columnvalue.get(Tables.USER.getName())).put(Tables.TAGS.getName(), (String)((HashMap)tables2Columnvalue.get(Tables.TAGS.getName())).get(Tables.TAGS.getCloumns()[1]));
            }
         }

         if (tables2Columnvalue.containsKey(Tables.TAGS.getName())) {
            tables2Columnvalue.remove(Tables.TAGS.getName());
         }

         if (tables2Columnvalue.containsKey(Tables.CATEGORY.getName())) {
            tables2Columnvalue.remove(Tables.CATEGORY.getName());
         }
      }
